...
std::wstring iTunesFrameworkSupport(const std::wstring dll_name) {
...
{
#ifdef WIN32
	m_hDll = DLOPEN(iTunesFrameworkSupport(L"CoreFoundation.dll").c_str());
...
CMobileDevice::CMobileDevice(Boolean newdll)
{
	...
	if (newdll)
	{
		m_hDll = DLOPEN(iTunesFrameworkSupport(L"MobileDevice.dll").c_str());
	}
	else
	{
...
{
#ifdef WIN32
	m_hDll = DLOPEN(iTunesFrameworkSupport(L"AirTrafficHost.dll").c_str());
